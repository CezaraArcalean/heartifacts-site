window.HeartifactsContent = window.HeartifactsContent || {};
window.HeartifactsContent.ro = {
  meta: {
    title: "Heartifacts — Alternative Metal din București",
    description: "Heartifacts este o trupă de alternative metal din București, cu hook-uri pop-punk, greutate hard-rock și piese trăite puțin prea intens."
  },
  nav: { music: "Muzică", live: "Gigs", about: "Despre", photos: "Poze", contact: "Contact", menu: "Meniu" },
  hero: {
    eyebrow: "Alternative metal · București",
    tagline: "pentru oamenii care simt totul puțin prea intens",
    positioning: "Alternative metal în cinci, cu hook-uri pop-punk, greutate hard-rock și piese încărcate emoțional."
  },
  quick: {
    follow: "Urmărește",
    listen: "Ascultă",
    watch: "Vezi",
    contact: "Contact",
    soon: "Link în curând"
  },
  actions: {
    listen: "Ascultă",
    watch: "Vezi",
    spotify: "Ascultă pe Spotify",
    youtube: "Vezi pe YouTube",
    tickets: "Bilete",
    details: "Mai multe detalii"
  },
  placeholders: {
    heroPhoto: "Aici vine fotografia principală",
    releaseArt: "Aici vine artwork-ul"
  },
  music: {
    kicker: "Muzica noastră",
    title: "Dă-i mai tare.",
    latest: "Cel mai recent release",
    body: "O piesă despre momentul în care îți dai seama că o persoană în care ai avut încredere nu e cine credeai.",
    releaseFallback: "Muzică nouă de la Heartifacts."
  },
  live: {
    kicker: "Gigs",
    title: "Concerte viitoare",
    dataFallback: "Detaliile concertelor apar în curând — urmărește-ne pe Instagram pentru anunțuri.",
    noShows: "Se gătesc concerte noi — urmărește-ne pe Instagram.",
    ticketsSoon: "Bilete / detalii în curând"
  },
  about: {
    kicker: "Despre noi",
    title: "Oameni: cinci\nTrupă: una\nMuzică: tare",
    body: "Suntem Heartifacts, cinci oameni din București care fac muzică pop-punk-ish pentru că, aparent, să le urlăm oamenilor în față tot ce simțim nu era o opțiune. Ne plac refrenele catchy, chitările date tare, energia haotică și câte un moment mai heavy când piesa îl cere. Pe parcurs, niște piese au reușit să scape din mințile noastre distrate și, până să ne prindem, deja n-aveam ce să le mai facem. Când nu suntem pe scenă, suntem ceva mai puțin dramatici: multe glume, distracție la repetiții și entuziasm care dă pe dinafară de fiecare dată când ne iese bine o piesă.",
    pullquote: "muzica o luăm în serios. pe noi, mai puțin."
  },
  photos: {
    kicker: "Dovezi",
    title: "Da, chiar existăm.",
    placeholderCaption: "Placeholder pentru fotografie de trupă"
  },
  socials: {
    kicker: "Și prin alte colțuri ale internetului",
    title: "Ne găsești și aici.",
    note: "Muzică, haos de repetiții și tot ce mai scapă din group chat."
  },
  footer: {
    location: "Alternative metal · București, România",
    booking: "Booking / Presă",
    contactLead: "Booking, presă sau pur și simplu vrei să ne scrii? Dă-ne un email sau un DM pe Instagram.",
    contactSoon: "Datele de booking apar în curând.",
    instagramDM: "Scrie-ne pe Instagram",
    findUs: "Ne găsești aici"
  }
};
